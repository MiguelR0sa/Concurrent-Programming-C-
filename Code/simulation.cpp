...
static int cpid[MAX_CLIENTS];   /* client process ids */
static int bpid[MAX_BARBERS];   /* barber process ids */
 ...
 */
int proc_create(void* (*routine)(void*), void* s)
{
    	
    int pid_t = pfork();
//atach
    /* parent side, even in case of error */
    if (pid_t != 0) return pid_t;
...
static void go()
{
   ...
   printf("Launching %d barber processes\n", global->NUM_BARBERS);
   for(int i = 0; i < global->NUM_BARBERS; i++)
     { log_barber(allBarbers+i);
      bpid[i] = proc_create(main_barber,(void*)(allBarbers+i))
   printf("- Barber process %d was launched\n", i);
    }
  printf("Launching %d client processes\n", global->NUM_CLIENTS);	
   for(int i = 0; i < global->NUM_CLIENTS; i++)
   {   log_client(allClients+i);
       cpid[i] = proc_create(main_client,(void*)(allClients+i))
       printf("- Client process %d was launched %d\n", i,cpid[i]);
   }
}
...
for (int i = 0; i < global->NUM_CLIENTS; i++)
    {
        pid_t pid;
        pid = pwaitpid(cpid[i], NULL, 0)
        printf("Client %d (process %d) has terminated\n", i, pid);
    }


    /* wait for barber processes to conclude */
    for (int i = 0; i < global->NUM_BARBERS; i++)
    {
        pid_t pid;
        pid = pwaitpid(bpid[i], NULL, 0)
     
        printf("Barber %d (process %d) has terminated\n", i, pid);
    }
   term_logger();
}
